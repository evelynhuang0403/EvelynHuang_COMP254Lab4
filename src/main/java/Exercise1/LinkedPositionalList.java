...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  private Position<E> position(Node<E> node) {
    ...
    return node; //position is the node itself
  }
  ...
  public E remove(Position<E> p) throws IllegalArgumentException {
    Node<E> node = validate(p); // ensure that p is a valid position
  ...
  //Iterable<T>	Represents a collection that can be looped over (e.g. in for-each)
  //Iterator<T>	Represents the actual walker, one step at a time
  private class PositionIterable implements Iterable<Position<E>> {
    // Returns an iterator for the positions collection.
  ...
  /**
   ...
   Write this method using only other methods of the PositionalList interface
  ...
  public int indexOf(Position<E> p) throws IllegalArgumentException {
    ...
    for (Position<E> pos : positions()) {
      if (pos.equals(p)) {
        return index; // Return the index if the position matches
      }
      index++; // Increase the index for each position traversed
    }

    // If finish the iteration without finding the position, throw an exception
    throw new IllegalArgumentException("Position not found in the list");
  }
}
